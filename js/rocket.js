...
function showCrashTimer() {
    const canvas = document.getElementById('rocketCanvas');
    const crashTimer = document.createElement('div');
    crashTimer.className = 'crash-timer';
    crashTimer.innerHTML = `
        <div class="crash-text">КРАХ</div>
        <div class="crash-multiplier">${rocketGame.crashPoint.toFixed(2)}x</div>
        <div class="countdown-timer">10</div>
    `;
    
    canvas.appendChild(crashTimer);
    
    let timeLeft = 10;
    const timerInterval = setInterval(() => {
        timeLeft--;
        const timerElement = crashTimer.querySelector('.countdown-timer');
        if (timerElement) {
